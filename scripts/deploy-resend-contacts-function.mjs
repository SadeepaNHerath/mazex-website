...
const functionDir = resolve(rootDir, "functions/sync-registration-contacts-to-resend");
const functionEnvPath = resolve(functionDir, ".env");
const archivePath = resolve(rootDir, ".tmp-registration-resend-contacts.tar.gz");
...
const functionId = "registration_resend_contacts";
const functionName = "Registration Resend Contacts";
...
async function deployArchive() {
  const deployment = await functions.createDeployment({
    functionId,
    code: InputFile.fromPath(archivePath, "registration-resend-contacts.tar.gz"),
    activate: true,
    entrypoint,
    commands,
  });
...
console.log("Resend contacts function deployment request submitted.");
